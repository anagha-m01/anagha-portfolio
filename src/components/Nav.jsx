  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-colors duration-300 ${
        scrolled ? "bg-bg/90 border-b border-hair-soft" : "border-b border-transparent"
      }`}
    >
      <nav className="mx-auto max-w-6xl px-5 sm:px-8 h-16 flex items-center justify-between">
        <a href="#top" className="font-display text-[15px] tracking-tight text-ink flex items-center gap-2.5">
          <span className="inline-flex h-2 w-2 rounded-full bg-teal" aria-hidden="true" />
          {profile.name}
        </a>
      ...
      {open && (
        <div className="md:hidden bg-bg border-b border-hair-soft px-5 pb-5">
